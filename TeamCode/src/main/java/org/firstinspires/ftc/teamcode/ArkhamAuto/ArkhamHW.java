...
public class ArkhamHW {
    ...
    public DigitalChannel InSwitch = null;
    public DigitalChannel OutSwitch = null;
    ...
    public void init(HardwareMap ahwMap) {
        ...
        Extendo = ahwMap.get(DcMotor.class, "e");
        ...
        IntakeServo = ahwMap.servo.get("is");
        TopSwitch = ahwMap.get(DigitalChannel.class,"t");
        BottomSwitch = ahwMap.get(DigitalChannel.class,"b");
        InSwitch = ahwMap.get(DigitalChannel.class,"isw");
        OutSwitch = ahwMap.get(DigitalChannel.class,"os");
    ...
    public void Retracto (double speed, double distance){
        Extendo.setTargetPosition((int) (-distance* 1 *  COUNTS_PER_INCH2));
        Extendo.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        Extendo.setPower(-speed);
    }

    public void Extendo (double speed, double distance){
        Extendo.setTargetPosition((int) (distance* 1 *  COUNTS_PER_INCH2));
        Extendo.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        Extendo.setPower(speed);
    }
    ...
    public boolean DriveDone(double distance){
        if ((Math.abs(LeftFrontMotor.getCurrentPosition() / COUNTS_PER_INCH) >= distance) ||
                ...
                (Math.abs(Extendo.getCurrentPosition() / COUNTS_PER_INCH) >= distance)
                )
        {
            return (true);
        } else return (false);
